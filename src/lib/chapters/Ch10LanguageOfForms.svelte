	...
	<div class="content-width">
		<div use:reveal>
			...
			<div class="chapter-epigraph">
				<blockquote>“Why do so many integral theorems say that the inside is determined by the boundary?”</blockquote>
		...
		<div class="neo-prose" use:reveal>
			<p>
				The Fundamental Theorem of Calculus says that integrating a derivative over an interval leaves
				only two endpoint values:
			</p>
		</div>
		...
		<div class="neo-prose" use:reveal>
			<p>
				...
				quantity measured around the boundary curve. In space, surface and volume theorems do the same.
				These are not coincidences. They are instances of one idea:
			</p>
		</div>
		...
		<div class="neo-prose" use:reveal>
			<p>
				To state that principle in every dimension, we need objects that know whether they are meant to be
				integrated along a path, across a surface, or through a volume. Those objects are
			...
			<ul>
				...
				<li>A <strong>1-form</strong> accepts one tangent vector. It measures along a directed step.</li>
				<li>A <strong>2-form</strong> accepts two tangent vectors. It measures oriented area.</li>
				<li>A <strong>3-form</strong> accepts three tangent vectors. It measures oriented volume.</li>
			</ul>
			<p>
				A 1-form on the plane looks like <Katex math={r`\omega=P\,dx+Q\,dy`} />. Applied at a point to a
				vector <Katex math={r`\mathbf v=(v_1,v_2)`} />, it gives
				<Katex math={r`\omega(\mathbf v)=Pv_1+Qv_2`} />. This resembles a dot product, but the form is
				the measuring rule and the vector is the motion being measured.
			</p>
			<p>
				A 2-form must be <strong>alternating</strong>: swapping its two input directions reverses the
				sign. The basic planar area form is <Katex math={r`dx\wedge dy`} />, and
			</p>
		</div>

		<div class="key-equation" use:reveal>
			<Katex
				math={r`(dx\wedge dy)(\mathbf v,\mathbf w)=v_1w_2-v_2w_1`}
				display
			/>
		</div>
		...
		<div class="neo-prose" use:reveal>
			<p>
				The right side is the signed area of the parallelogram spanned by the two vectors. Before moving
				the controls below, predict what happens when the vectors become parallel, and what happens when
		...
		<div class="neo-prose" use:reveal>
			<p>
				...
				<Katex math={r`f(x,y)\,dx\wedge dy`} /> measures signed area with density <Katex math="f" />.
			</p>
			...
			<p>
				...
				<Katex math={r`\omega(v_1,v_2)=-yv_1+xv_2`} />. On a circle centered at the origin, this form
				measures motion in the tangential direction; radial motion contributes zero.
			</p>

			<h3>The exterior derivative measures local boundary change</h3>
			<p>
				The exterior derivative raises degree by one. It turns a quantity measured on boundaries into a
				density measured in interiors. For a function <Katex math={r`f(x,y)`} />,
			</p>
		</div>
		...
		<div class="neo-prose" use:reveal>
			<p>
				...
				loop reads how much the 1-form turns around it; dividing by the ordinary loop area and taking the
				classical limit leads to this coefficient. In an SDG treatment, the same coordinate formula is
		...
		<div class="neo-prose" use:reveal>
			<p>
				Thus the circulation of <Katex math="\omega" /> around a suitable planar boundary equals twice the
				signed area inside it. That sentence is already Green’s theorem for this particular form.
			</p>
		</div>
		...
		<Callout type="warning" title="Two different meanings of d² = 0">
			<p>
				...
				<strong>distinct, typed statements</strong>: one concerns an infinitesimal displacement; the other
				concerns an operator on forms. SDG relates their geometry, but identical notation does not make
				them the same axiom.
			</p>
		</Callout>
		...
		<div class="neo-prose" use:reveal>
			<p>
				For a twice continuously differentiable function, the operator identity follows from equality of
				mixed partials:
			</p>
		</div>
		...
		<DigDeeper title="Why d raises degree">
			<p>
				A function changes along directed steps, so its derivative is a 1-form. A 1-form changes around
				tiny oriented loops, so its derivative is a 2-form. A 2-form changes across the faces of tiny
				oriented boxes, so its derivative is a 3-form. The degree shift records the passage from a
				boundary measurement to an interior density.
			</p>
		</DigDeeper>
		...
		<div class="neo-prose" use:reveal>
			<p>
				Orientation is essential. Two neighboring cells traverse their shared face in opposite directions,
				so the two boundary contributions cancel. Only faces on the outer boundary remain. Change the
				grid size below and inspect one shared edge carefully.
			</p>
		</div>
		...
		<div class="neo-prose" use:reveal>
			<p>
				<strong>Takeaway.</strong> Cancellation is the geometric heart of Stokes’ theorem, but it is not by
				itself a complete proof for arbitrary smooth regions. A classical proof controls orientations,
				charts or triangulations, limits, and regularity. We do not build a finite region from
				“infinitely many infinitesimal cells.”
			</p>

			<h3>Four theorems, one sentence</h3>
			<p>
				Choose each theorem in the explorer. Track three things: the dimension of the region, the object
				integrated over its boundary, and the derivative integrated through its interior.
			</p>
		</div>
		...
		<div class="neo-prose" use:reveal>
			<p>
				<strong>Takeaway.</strong> The symbol <Katex math="d" /> changes its coordinate appearance with the
				degree of the form, while Stokes’ theorem itself does not change.
			</p>
			...
			<p>
				The converse is local under the hypotheses of the Poincaré lemma, and it is global on domains such
				as star-shaped regions. Holes can obstruct a global primitive. The most vivid example on the
				punctured plane is
			</p>
		</div>
		...
		<div class="neo-prose" use:reveal>
			<p>
				Away from the origin, <Katex math={r`d\omega=0`} />. Yet every positively oriented circle around the
				missing point has <Katex math={r`\oint\omega=2\pi`} />. Move the loop radius below. The field
				weakens with distance, but the path becomes longer in exactly the compensating way.
			</p>
		</div>
		...
		<div class="neo-prose" use:reveal>
			<p>
				<strong>Takeaway.</strong> If this form were exact, its integral around every closed loop would be
				zero by Stokes’ theorem. The nonzero loop integral proves that no single-valued global potential
				exists on the punctured plane. The quotient “closed forms modulo exact forms” begins de Rham
				cohomology, where calculus detects topology.
			</p>
			...
			<Exercise number={1}>
				...
				{#snippet solution()}<p>
					<Katex math={r`2\cdot3-1\cdot(-1)=7`} />. The oriented area is 7, and the positive sign means
					the turn from <Katex math="\mathbf v" /> to <Katex math="\mathbf w" /> is counterclockwise.
				</p>{/snippet}
			</Exercise>
			...
			<Exercise number={6}>
				<p>
					Why do two neighboring oriented cells contribute opposite values on their shared edge? Why does
					this explanation alone not prove Stokes’ theorem for every smooth region?
				</p>
				{#snippet solution()}<p>
					The induced boundary orientations on a shared edge are opposite, so the two integrals cancel. A
					general proof must additionally control a decomposition or charts, limiting processes,
					orientation consistency, and regularity.
				</p>{/snippet}
			</Exercise>
		...
		<ChapterSummary>
			<ul>
				...
				<li>The wedge product builds higher-degree forms; <Katex math={r`dx\wedge dy`} /> measures signed area.</li>
				<li>The exterior derivative raises degree: <Katex math={r`f\mapsto df`} /> and <Katex math={r`\omega\mapsto d\omega`} />.</li>
				<li><Katex math={r`d\circ d=0`} /> for the exterior derivative is distinct from a nilpotent displacement satisfying <Katex math={r`\delta^2=0`} />.</li>
				<li>Generalized Stokes says <Katex math={r`\int_Md\omega=\int_{\partial M}\omega`} /> under the appropriate smoothness and orientation hypotheses.</li>
				<li>FTC, Green, classical Stokes, and divergence are dimension-specific readings of the same theorem.</li>
		...
		<div class="neo-prose" use:reveal>
			...
			<p>
				The book began by asking how a curve responds to a first-order displacement. It ends with the same
				local-to-global movement in a wider language. Derivatives describe local response. Integrals
				accumulate it. Orientation determines signs. Boundaries record what survives cancellation.
			</p>
			<p>
				Synthetic differential geometry supplies an exact first-order lens for smooth local behavior.
				Classical integration supplies the global accumulation principles and analytic hypotheses. Used
				together—and kept carefully distinct—they reveal a remarkably coherent calculus.
			</p>
		</div>

		<Callout type="key-idea" title="The final sentence">
			<p>
				Calculus is the study of how local change accumulates, and Stokes’ theorem says that the accumulated
				change inside a region is written on its boundary.
			</p>
		</Callout>
	</div>
</section>
